use quijine::{Class, ClassMethods, Context, EvalFlags};
...
fn new_object_class() {
    quijine::run_with_context(|ctx| {
        ...
        let result = ctx
            .eval("const s1 = S1(); s1", "<input>", EvalFlags::TYPE_GLOBAL)
            .unwrap();
        assert!(!result.prototype().is_null());
        let name = ctx.eval("s1.name()", "<input>", EvalFlags::TYPE_GLOBAL).unwrap();
        assert_eq!("foo", name.to_string().unwrap());
        let name = ctx
            .eval("s1.setName('bar'); s1.name()", "<input>", EvalFlags::TYPE_GLOBAL)
            .unwrap();
        assert_eq!("bar", name.to_string().unwrap());
        let x = ctx.eval("s1.pos().x", "<input>", EvalFlags::TYPE_GLOBAL).unwrap();
        assert_eq!(3, x.to_i32().unwrap());
        let y = ctx.eval("s1.pos().y", "<input>", EvalFlags::TYPE_GLOBAL).unwrap();
        assert_eq!(4, y.to_i32().unwrap());
        ctx.eval("s1.move(2, 3)", "<input>", EvalFlags::TYPE_GLOBAL).unwrap();
        let x = ctx.eval("s1.pos().x", "<input>", EvalFlags::TYPE_GLOBAL).unwrap();
